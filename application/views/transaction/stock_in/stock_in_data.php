...
<nav class="navbar navbar-main navbar-expand-lg px-0 mx-4 shadow-none border-radius-xl" id="navbarBlur" navbar-scroll="true">
    <div class="container-fluid py-1 px-3">
        <nav aria-label="breadcrumb">
            <h6 class="font-weight-bolder mb-0 mt-0">Barang Masuk</h6>
        </nav>
        ...
        <div class="col-12">
            <div class="card mb-4">
                <div class="card-header pb-0">
                    <div class="row">
                        <div class="col">
                            <h6>Data Barang yang Masuk</h6>
                        </div>
                        <div class="col">
                            <div class="float-end">
                                <a class="btn btn-sm bg-gradient-primary" href="<?= site_url('stock/in/add') ?>">Tambah Barang Masuk</a>
                            </div>
                        </div>
                    </div>
                </div>
                        ...
                        <table class="table align-items-center mb-0" id="table">
                            <thead>
                                <tr>
                                    ...
                                    <th class="text-center text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">Tanggal Barang Masuk</th>
                                ...
                                foreach ($row as $key => $data) { ?>
                                    <tr>
                                        ...
                                        <td class="align-middle text-center text-sm">
                                            <button type="button" class="btn badge badge-sm bg-gradient-secondary no-mb" data-toggle="tooltip" title="Detail Barang Masuk" data-bs-toggle="modal" data-bs-target="#modal-detail<?= $data->stock_id ?>"><i class="fa fa-eye"></i></button>
                                            <button type="button" class="btn badge badge-sm bg-gradient-danger no-mb" data-toggle="tooltip" title="Hapus Barang Masuk" data-bs-toggle="modal" data-bs-target="#modal-hapus<?= $data->stock_id ?>"><i class="fa fa-trash"></i></button>
                                        </td>
                                    </tr>
                                <?php } ?>
                            </tbody>
                    ...
                    <table class="table table-bordered no-mb">
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td><?= wordwrap($data->supplier_name, 55, "<br>") ?></span></td>
                            </tr>
                            <tr>
...
<?php foreach ($row as $key => $data) { ?>
    <div class="modal fade" id="modal-hapus<?= $data->stock_id ?>" tabindex="-1" role="dialog">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <?php echo form_open('stock/in/delete/' . $data->stock_id . '/' . $data->item_id) ?>
                <div class="modal-header">
                    <h5 class="modal-title">Hapus Data Barang Masuk</h5>
                    <button type="button" class="btn badge bg-gradient-secondary no-mb" data-bs-dismiss="modal" data-toggle="tooltip" title="Tutup"><i class="fa fa-times"></i></button>
                </div>
                <div class="modal-body">
                    <p class="no-mb">Yakin Akan Menghapus Data Barang Masuk <?php echo $data->item_name ?>?</p>
                </div>
                <div class="modal-footer">
                    <button type="submit" class="btn btn-sm bg-gradient-primary no-mb float-end">Hapus Data</button>
                </div>
                <?php echo form_close(); ?>
            </div>
        </div>
