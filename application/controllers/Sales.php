...
class Sales extends CI_Controller
{
	...
	public function index()
	{
		...
		$this->template->load('backend/template', 'transaction/sales/sales_form', $data);
	}
	...
	public function process()
	{
		...
		if (isset($_POST['add_cart'])) {
			...
			$check_cart = $this->sales_m->get_cart(['transaction_cart.item_id' => $item_id])->num_rows();
			if ($check_cart > 0) {
	...
	public function cetak($id)
	{
		...
		$kertas = array(0, 0, 300.00, 650.00);
		/* $this->pdf->set_option('dpi', 72); */
		$this->pdf->setPaper($kertas, 'portrait');
		$this->pdf->load_view("transaction/sales/receipt_print", $data);
	}
